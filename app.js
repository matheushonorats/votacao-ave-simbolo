 ...
 * Lógica 100% Inline — Sem modais popup que saem do campo de visão em iframes.
 */
      ...
      <div class="showcase-overlay">
        <div class="showcase-content">
          ...
          <div class="showcase-actions">
            <button type="button" class="showcase-btn-info" onclick="showcaseExpandBird('${bird.id}')">Conhecer a espécie</button>
...
// ============================================================
// RENDERIZAÇÃO DO GRID DE AVES COM EXPANSÃO INLINE
// ============================================================
    ...
    <article class="bird-card" id="card-${bird.id}" data-bird-id="${bird.id}">
      ...
      <div class="bird-card__body">
        ...
        <div class="bird-card__actions">
          <button type="button" class="bird-card__btn-info" id="btnInfo-${bird.id}" onclick="toggleBirdDetails('${bird.id}')">
            <span>Conhecer</span>
          </button>
          <button type="button" class="bird-card__btn-vote" id="btnVote-${bird.id}" onclick="selectBird('${bird.id}', true)">Votar</button>
        </div>
      </div>
      
      <!-- Drawer de Detalhes Inline (100% no campo de visão) -->
      <div class="bird-card__details-drawer" id="drawer-${bird.id}">
        <h4 class="bird-card__details-title">Por que votar nesta espécie?</h4>
        <div class="bird-card__details-content">
          ${bird.description}
        </div>
        <div class="bird-card__details-actions">
          <button type="button" class="btn btn-secondary" onclick="toggleBirdDetails('${bird.id}')">Recolher</button>
          <button type="button" class="btn btn-primary" onclick="selectBird('${bird.id}', true)">Escolher esta Ave</button>
        </div>
      </div>
    </article>
  `).join('');
...
// ============================================================
// EXPANSÃO INLINE DOS DETALHES DA AVE
// ============================================================
function toggleBirdDetails(birdId) {
  ...
  // Fecha qualquer outro card expandido para manter a interface limpa
  document.querySelectorAll('.bird-card.is-expanded').forEach(c => {
    if (c.id !== `card-${birdId}`) {
      c.classList.remove('is-expanded');
      const infoBtn = c.querySelector('.bird-card__btn-info span');
      if (infoBtn) infoBtn.textContent = 'Conhecer';
    }
  });

  // Alterna o estado do card clicado
  card.classList.toggle('is-expanded', !isExpanded);
  const infoBtn = card.querySelector('.bird-card__btn-info span');
  if (infoBtn) {
...
function showcaseExpandBird(birdId) {
  const card = document.getElementById(`card-${birdId}`);
  if (!card) return;
  ...
  notifyParentToScroll((window.pageYOffset || document.documentElement.scrollTop || 0) + rect.top - 20);
}
...
// ============================================================
// EXIBIÇÃO DO RESULTADO DO VOTO (INLINE)
// ============================================================
function showSuccessInline(bird, email) {
  const votingCard = document.getElementById('votingCard');
  const successCard = document.getElementById('votingSuccessCard');
...
async function handleVoteSubmit(e) {
  ...
  if (!selectedBirdId) {
    showInlineAlert('Nenhuma Ave Selecionada', 'Por favor, escolha uma das seis espécies candidatas acima antes de confirmar seu voto.');
    return;
  }
