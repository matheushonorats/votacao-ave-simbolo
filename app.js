...
function selectBird(birdId, scrollToVote = false) {
  if (CONFIG.STATUS_VOTACAO === 'ENCERRADA') {
    openAlertModal('Votação Encerrada', 'A votação oficial está encerrada.');
    return;
  }
...
// ============================================================
// POSICIONAMENTO DINÂMICO DE MODAL NO CAMPO DE VISÃO
// ============================================================
function positionModalInCurrentView(modalElement, targetElement) {
  ...
  if (targetElement) {
    ...
    scrollY = (window.pageYOffset || document.documentElement.scrollTop || 0) + rect.top - 20;
  } else {
    scrollY = window.pageYOffset || document.documentElement.scrollTop || 0;
  }

  // Define a posição absoluta do modal diretamente onde o usuário está olhando no momento
  modalElement.style.top = Math.max(0, scrollY) + 'px';
...
function openBirdModal(birdId) {
  ...
  positionModalInCurrentView(modal, cardElement);
  ...
  if (cardElement) {
    ...
    const targetScrollY = (window.pageYOffset || document.documentElement.scrollTop || 0) + rect.top - 40;
...
function openSuccessModal(bird, email) {
  ...
  positionModalInCurrentView(modal, votingSection);
  ...
  if (votingSection) {
    ...
    notifyParentToScroll((window.pageYOffset || document.documentElement.scrollTop || 0) + rect.top);
  }
}
...
// ============================================================
// MODAL & BANNER DE ALERTA EM ALTO DESTAQUE
// ============================================================
